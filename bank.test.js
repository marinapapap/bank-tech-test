...
describe("Bank class", () => {
  ...
  it("adds a deposit value with the date and returns data in a 3 element array (positons 1 and 0 respectively)", () => {
    ...
    expect(bank.returnRecords()[0]).toEqual({ date: "13/01/2023", deposit: 1000, withdrawal: "", balance: 1000 });
  ...
  it("adds a withdrawal value with the date and returns data in a 3 element array (positons 2 and 0 respectively)", () => {
    ...
    expect(bank.returnRecords()[0]).toEqual({ date: "13/01/2023", deposit: "", withdrawal: 500, balance: -500 });
  ...
  it("adds deposits and a withdrawal value with dates and returns data as an array of objects", () => {
    ...
    expect(bank.returnRecords()[0]).toEqual({ date: "10/01/2023", deposit: 1000, withdrawal: "", balance: 1000 });
    expect(bank.returnRecords()[2]).toEqual({ date: "14/01/2023", deposit: "", withdrawal: 500, balance: 2500 });
  ...
  it("adds another withdrawal after returning the records, then returns the records again to see if the new record is at the end of the records array", () => {
    ...
    expect(bank.returnRecords()[3]).toEqual({ date: "17/01/2023", deposit: "", withdrawal: 500, balance: 2000 });
  });
});